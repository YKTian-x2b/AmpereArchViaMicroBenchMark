...
__global__ void SMem_writeBW_test_kernel(uint32_t *startClk, uint32_t *stopClk, float *out){
    ...
    startClk[tid] = start;
    stopClk[tid]= stop;
    for(int i = tid; i < SMEM_SIZE; i += BLK_SIZE){
        out[tid] += Arr[i];
    }
    // float tmp = Arr[tid];
    // if(tmp == 0){
...
// nvcc -gencode=arch=compute_86,code=\"sm_86,compute_86\" -I../Utils -L /usr/local/cuda/lib64 -l cuda -o res/smem_bandwidth smem_bandwidth.cu

// nvcc --keep --keep-dir midRes_1 -gencode=arch=compute_86,code=\"sm_86,compute_86\" -I../Utils -L /usr/local/cuda/lib64 -l cuda -o res/smem_bandwidth smem_bandwidth.cu
// cuasm --bin2asm midRes_1/smem_bandwidth.sm_86.cubin -o midRes_1/smem_bandwidth.sm_86.cuasm

// nvcc --keep --keep-dir midRes_1_1 -gencode=arch=compute_86,code=\"sm_86,compute_86\" -I../Utils -L /usr/local/cuda/lib64 -l cuda -o res/smem_bandwidth smem_bandwidth.cu
// cuasm --bin2asm midRes_1_1/smem_bandwidth.sm_86.cubin -o midRes_1_1/smem_bandwidth.sm_86.cuasm
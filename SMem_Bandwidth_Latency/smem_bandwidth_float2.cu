...
__global__ void SMem_writeBW_test_kernel(uint32_t *startClk, uint32_t *stopClk, float *out){
    int tid = threadIdx.x;
    __shared__ float2 Arr[SMEM_SIZE+BLK_SIZE]; // 很极限
    float2 reg = make_float2(tid+3.0, tid+4.0);
    Arr[tid] = reg;     /// 为了让计时代码中没有sts之外的指令，这里提前准备好reg变量。
...
int main(){
    ...
    uint32_t BytesAll = SMEM_SIZE * sizeof(float2) * (BLK_SIZE/WARP_SIZE);  // * (BLK_SIZE/WARP_SIZE);   //  * BLK_SIZE;
    //
...
// nvcc --keep --keep-dir bw_midRes_float2 -gencode=arch=compute_86,code=\"sm_86,compute_86\" -I../Utils -L /usr/local/cuda/lib64 -l cuda -o res/smem_bandwidth_float2 smem_bandwidth_float2.cu
// cuasm --bin2asm bw_midRes_float2/smem_bandwidth_float2.sm_86.cubin -o bw_midRes_float2/smem_bandwidth_float2.sm_86.cuasm
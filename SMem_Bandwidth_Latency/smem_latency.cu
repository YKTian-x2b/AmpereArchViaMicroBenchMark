...
void smem_st_latency_test_kernel(uint32_t *startClk, uint32_t *stopClk, uint32_t *out){
    ...
    //     asm volatile (
    //         "st.shared.b32 [%0], %1;\n"
    //         : "=r"(arr_addr) : "r"(tmp) : "memory"
    //     );
    // } //  + ((i*4)%SMEM_SIZE)
    asm volatile (
        "st.shared.b32 [%0], %1;\n"
        : "=r"(arr_addr) : "r"(16) : "memory"
    );
...
// nvcc -gencode=arch=compute_86,code=\"sm_86,compute_86\" -I../Utils -L /usr/local/cuda/lib64 -l cuda -o res/smem_latency smem_latency.cu

// nvcc --keep --keep-dir latency_midRes -gencode=arch=compute_86,code=\"sm_86,compute_86\" -I../Utils -L /usr/local/cuda/lib64 -l cuda -o res/smem_latency smem_latency.cu
// cuasm --bin2asm latency_midRes/smem_latency.sm_86.cubin -o latency_midRes/smem_latency.sm_86.cuasm

// cuasm --asm2bin latency_midRes/smem_latency.sm_86.cuasm -o latency_midRes/smem_latency.sm_86.cubin

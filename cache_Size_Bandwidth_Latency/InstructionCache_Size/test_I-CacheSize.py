...
def build(use_driver_api=True):
    cuasm2cubin(projectPath + 'cache_Size_Bandwidth_Latency/InstructionCache_Size/midRes/I-CacheSizeTest.sm_86.cuasm')
...
def run_exe():
    try:
        res = subprocess.check_output(projectPath + 'cache_Size_Bandwidth_Latency/InstructionCache_Size/res/I-CacheSizeTest')
        return res
...
def test_L0CacheSize(n_FFMA_seqs):
    cat = CuAsmTemplate(projectPath + 'cache_Size_Bandwidth_Latency/InstructionCache_Size/res/I-CacheSizeTest.template.sm_86.cuasm')
    ...
    cat.generate(projectPath + 'cache_Size_Bandwidth_Latency/InstructionCache_Size/midRes/I-CacheSizeTest.sm_86.cuasm')
...
def doTest_L0CacheSize():
    with open(projectPath + 'cache_Size_Bandwidth_Latency/InstructionCache_Size/res/res_I-CacheSize.txt', 'w') as fout:
        ...
        # # L2 detect的时候上限应该差不多是65536*4。但实际做出来的结果和Unified L2 cache有很大偏差
...
def draw():
    with open(projectPath + 'cache_Size_Bandwidth_Latency/InstructionCache_Size/res/res_I-CacheSize.txt', 'r') as file:
        file.readline()
        res = file.read()
        key_list = []
        val_list = []

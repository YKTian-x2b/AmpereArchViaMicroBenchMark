...
int main(){
    // 为了让kernel的sass中ldg指令占比尽量高，我们把kernel写的干净一点，尾数据(BLK_SIZE)可以保证每个thread读取全部数据而不出现问题
...
// 512 : L1 Data cache Bandwidth is  62.1288 Bytes per cycle per SM
// 1024: L1 Data cache Bandwidth is  65.2702 Bytes per cycle per SM
...
__global__ void L0CacheSize_test_kernel(const int2 c, const float4 v, float* a)
{
    int tid = threadIdx.x;
    int bid = blockIdx.x;
    int wid = tid / 32;
...
float L0CacheSize_test_run(const int2 c, const float4 v, float* a, cudaEvent_t &event_start, cudaEvent_t &event_stop)
{
    float elapsedTime;
...
float L0CacheSize_test_run_drv(const int2 c, const float4 v, float* a, cudaEvent_t &event_start, cudaEvent_t &event_stop)
{
    ...
    if (!isInitialized)
    {
        ...
        cuModuleGetFunction(&kernel, cuModule, "_Z19L0CacheSize_test_kernel4int2i6float4Pf");
...
void dotest()
{
    ...
    {
        da.SetZeros();
        // float elapsedTime = L0CacheSize_test_run_drv(c, NIter, v, da.GetPtr(), event_start, event_stop); // in ms
        float elapsedTime = L0CacheSize_test_run(c, v, da.GetPtr(), event_start, event_stop);

...
# 结论是 Ampere的寄存器还是两个bank

# baseline result for:
# -  grid=7200, block=128, NIter=256, 192 instruction per iteration
# -  No bank conflict
# -  MemFreq = 7200 MHz, SMFreq = 1700 MHz
# 30*128*1700 = 6528G 指令/s
...
def build(use_driver_api=True):
    cuasm2cubin('/opt/kaiProjects/GEMM_kai/MicroBenchmark/regBank2/regBankTest.sm_86.cuasm')
...
def template_test():
    cat = CuAsmTemplate('/opt/kaiProjects/GEMM_kai/MicroBenchmark/regBank2/save/regBankTest.template.sm_86.cuasm')
...
def run_exe():
    try:
        res = subprocess.check_output('/opt/kaiProjects/GEMM_kai/MicroBenchmark/regBank2/res/regBankTest')
        return res
...
def test_NoBankConflict(stall1, stall2, yflag, regnum):
    ...
    cat.generate('/opt/kaiProjects/GEMM_kai/MicroBenchmark/regBank2/regBankTest.sm_86.cuasm')
...
if __name__ == '__main__':
    ...
    os.system('nvidia-smi -ac 7000,1700')

...
sys.path.append('/opt/kaiProjects/MicroBenchmark/Utils')
...
def build(use_driver_api=True):
    cuasm2cubin('/opt/kaiProjects/MicroBenchmark/warpSchedule/warpScheduleTest.sm_86.cuasm')
...
def run_exe():
    try:
        res = subprocess.check_output('/opt/kaiProjects/MicroBenchmark/warpSchedule/warpScheduleTest')
        return res
...
def test_warpSchedule(n_FFMA_seqs):
    cat = CuAsmTemplate('/opt/kaiProjects/MicroBenchmark/warpSchedule/save/warpScheduleTest.template.sm_86.cuasm')
    ...
    # s_work1  = f'      [B------:R-:W-:Y:S04]    FFMA R9, R9, R4, c[0x0][0x164] ;\n'
    # s_work1  = f'      [B------:R-:W-:Y:S04]    FFMA R9, R9, R4, c[0x0][0x16c] ;\n'
    #
    s_work1 = ''
    for i in range(4):
    ...
    cat.generate('/opt/kaiProjects/MicroBenchmark/warpSchedule/warpScheduleTest.sm_86.cuasm')
...
def doTest_warpSchedule():
    n_FFMA_seqs = 4096
    test_warpSchedule(n_FFMA_seqs)
    # with open('/opt/kaiProjects/MicroBenchmark/warpSchedule/save/res_warpScheduleTest.txt', 'w') as fout:
